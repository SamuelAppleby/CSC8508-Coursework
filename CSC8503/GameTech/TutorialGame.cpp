...
void TutorialGame::UpdateLevel(float dt)
{
	...
	if (WorldCreator::GetLockedObject() != nullptr)
	{
	...
	else if (WorldCreator::GetSelectionObject()) {
		DebugObjectMovement();
	}
}
...
void TutorialGame::InitFloors(LevelState state)
{
	...
	case LevelState::LEVEL2:
		...
		//diving board centre
		WorldCreator::AddPxFloorToWorld(PxTransform(PxVec3(0, 45, -1495)), PxVec3(20, 1, 70), 0.5, 2);
...
void TutorialGame::InitGameObstacles(LevelState state)
{
	...
	case LevelState::LEVEL2:
		...
		WorldCreator::AddPxRotatingCubeToWorld(PxTransform(PxVec3(0, -98, -900)), PxVec3(20, 20, 198), PxVec3(0, 0, 1));

		//WorldCreator::AddPxFloorToWorld(PxTransform(PxVec3(70, -98, -900)), PxVec3(20, 20, 200));
		WorldCreator::AddPxRotatingCubeToWorld(PxTransform(PxVec3(70, -98, -900)), PxVec3(20, 20, 198), PxVec3(0, 0, 1));

		//cannons
		WorldCreator::AddPxCannonToWorld(PxTransform(PxVec3(-150, -80, -850)), PxVec3(7000000, 8500, 0), 10, 10);
		WorldCreator::AddPxCannonToWorld(PxTransform(PxVec3(-150, -80, -900)), PxVec3(7000000, 8500, 0), 10, 10);
		WorldCreator::AddPxCannonToWorld(PxTransform(PxVec3(-150, -80, -950)), PxVec3(7000000, 8500, 0), 10, 10);

		WorldCreator::AddPxCannonToWorld(PxTransform(PxVec3(150, -80, -825)), PxVec3(7000000, 8500, 0), 10, 10);
		WorldCreator::AddPxCannonToWorld(PxTransform(PxVec3(150, -80, -875)), PxVec3(7000000, 8500, 0), 10, 10);
		WorldCreator::AddPxCannonToWorld(PxTransform(PxVec3(150, -80, -925)), PxVec3(7000000, 8500, 0), 10, 10);
		WorldCreator::AddPxCannonToWorld(PxTransform(PxVec3(150, -80, -975)), PxVec3(7000000, 8500, 0), 10, 10);
		...
		WorldCreator::AddPxRotatingCubeToWorld(PxTransform(PxVec3(0, -78, -1700)), PxVec3(190, 20, 20), PxVec3(0, 1, 0));
		WorldCreator::AddPxCannonToWorld(PxTransform(PxVec3(-80, 100, -1351)), PxVec3(1, 1, 70000000), 30, 15);
		WorldCreator::AddPxCannonToWorld(PxTransform(PxVec3(-40, 100, -1351)), PxVec3(1, 1, 70000000), 30, 15);
		WorldCreator::AddPxCannonToWorld(PxTransform(PxVec3(0, 100, -1351)), PxVec3(1, 1, 70000000), 30, 15);
		WorldCreator::AddPxCannonToWorld(PxTransform(PxVec3(40, 100, -1351)), PxVec3(1, 1, 70000000), 30, 15);
		WorldCreator::AddPxCannonToWorld(PxTransform(PxVec3(80, 100, -1351)), PxVec3(1, 1, 70000000), 30, 15);
		break;
	}
...
bool TutorialGame::SelectObject()
{
	if (Window::GetMouse()->ButtonDown(NCL::MouseButtons::LEFT) && !WorldCreator::GetLockedObject())
	{
		...
		if (pXPhysics->GetGScene()->raycast(pos, dir, distance, hit))
		{
			...
			if (WorldCreator::GetSelectionObject()->GetRenderObject())
				WorldCreator::GetSelectionObject()->GetRenderObject()->SetColour(Vector4(0, 1, 0, 1));
			return true;
		}
		return false;
	}
	...
	if (Window::GetKeyboard()->KeyPressed(NCL::KeyboardKeys::L))
	{
		if (WorldCreator::GetSelectionObject())
		{
			if (WorldCreator::GetLockedObject() == WorldCreator::GetSelectionObject())
			{
				WorldCreator::SetCamMode(CameraState::FREE);
			...
			else
			{
				...
				WorldCreator::SetLockedObject(WorldCreator::GetSelectionObject());
				Window::GetWindow()->ShowOSPointer(false);
				Window::GetWindow()->LockMouseToWindow(true);
			}
			world->GetMainCamera()->SetState(WorldCreator::GetCameraState());
...
void TutorialGame::LockedObjectMovement(float dt)
{
	...
	fwdAxis.y = 0.0f;
	fwdAxis.Normalise();
	Vector3 charForward = Quaternion(WorldCreator::GetLockedObject()->GetTransform().GetOrientation()) * Vector3(0, 0, 1);
	float force = 500000.0f;

	if (WorldCreator::GetLockedObject()->GetPhysicsObject()->GetPXActor()->is<PxRigidDynamic>())
	{
		PxRigidDynamic* body = (PxRigidDynamic*)WorldCreator::GetSelectionObject()->GetPhysicsObject()->GetPXActor();
		...
		if (Window::GetKeyboard()->KeyDown(KeyboardKeys::D))
			body->addForce(PhyxConversions::GetVector3(rightAxis) * force * dt, PxForceMode::eIMPULSE);
		if (Window::GetKeyboard()->KeyPressed(KeyboardKeys::SPACE) && WorldCreator::GetLockedObject()->IsGrounded())
		{
	...
	if (Window::GetKeyboard()->KeyPressed(NCL::KeyboardKeys::NUM1))
	{
		...
		case CameraState::THIRDPERSON:
			WorldCreator::SetCamMode(CameraState::TOPDOWN);
			break;
